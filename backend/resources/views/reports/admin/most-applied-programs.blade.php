...
<head>
    ...
    <style>
        ...
        .bar-container { background: #e2e8f0; border-radius: 10px; height: 12px; overflow: hidden; min-width: 120px; }
        .bar-fill { background: #2D6A4F; height: 100%; border-radius: 10px; }
...
<body>
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="filter-info">Periode Pendaftaran: {{ $filters['tgl_mulai'] ?? '...' }} s/d {{ $filters['tgl_akhir'] ?? '...' }}</div>
            @endif
        </div>
        ...
        <table>
            <thead>
                <tr>
                    <th>No</th>
                    <th>Nama Program</th>
                    <th class="text-right">Pendaftar Unik</th>
            ...
            <tbody>
                ...
                @php $persen = $row['quota_total'] > 0 ? round(($row['total_pendaftar_unik'] / $row['quota_total']) * 100) : 0; @endphp
                <tr>
                    <td class="text-center">{{ $i + 1 }}</td>
                    <td>{{ $row['program'] }}</td>
                    <td class="text-right">{{ number_format($row['total_pendaftar_unik'], 0, ',', '.') }}</td>
                    <td class="text-right">{{ number_format($row['quota_total'], 0, ',', '.') }}</td>
                    <td class="text-right">{{ $persen }}%</td>
                    <td>
                        <div class="bar-container">
                            <div class="bar-fill" style="width:{{ min($persen, 100) }}%;"></div>
                        </div>
                    </td>
                </tr>
                @endforeach
