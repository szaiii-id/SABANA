...
<head>
    ...
    <style>
        ...
        .badge-revoked { background: #fee2e2; color: #991b1b; padding: 3px 8px; border-radius: 10px; font-size: 9px; font-weight: bold; }
        .alasan { max-width: 150px; font-style: italic; color: #991b1b; }
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Desa</th>
                    <th>Program</th>
                    <th class="text-right">Jumlah Sebelumnya (Rp)</th>
                    <th class="alasan">Alasan Penghentian</th>
                    <th>Tgl Keputusan</th>
                    <th>Pemutus</th>
                </tr>
            </thead>
            <tbody>
                ...
                @php $total += $row['jumlah_sebelumnya']; @endphp
                <tr>
                    ...
                    <td>{{ $row['desa'] ?? '—' }}</td>
                    <td>{{ $row['program'] }}</td>
                    <td class="text-right">{{ number_format($row['jumlah_sebelumnya'], 0, ',', '.') }}</td>
                    <td class="alasan">{{ $row['alasan'] ?? '—' }}</td>
                    <td>{{ $row['tgl_keputusan'] }}</td>
                    <td>{{ $row['pemutus'] ?? '—' }}</td>
                </tr>
                @endforeach
                <tr class="total-row">
                    <td colspan="5" class="text-right"><strong>TOTAL: {{ count($data) }} orang dihentikan</strong></td>
