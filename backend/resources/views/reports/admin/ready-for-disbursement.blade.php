...
<head>
    ...
    <style>
        ...
        .badge-ready { background: #fef3c7; color: #92400e; padding: 3px 8px; border-radius: 10px; font-size: 9px; font-weight: bold; }
        .catatan-kosong { width: 100px; }
...
<body>
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="filter-info">Periode Validasi: {{ $filters['tgl_mulai'] ?? '...' }} s/d {{ $filters['tgl_akhir'] ?? '...' }}</div>
            @endif
        </div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Tgl Divalidasi</th>
                    <th>Validator</th>
                    <th>Keterangan</th>
                    <th class="catatan-kosong">Catatan</th>
                </tr>
            </thead>
            <tbody>
                @php $total = 0; @endphp
                @foreach($data as $i => $row)
                @php $total += $row['benefit_amount']; @endphp
                <tr>
                    ...
                    <td>{{ $row['tgl_divalidasi'] }}</td>
                    <td>{{ $row['validator'] ?? '—' }}</td>
                    <td><span class="badge-ready">Siap Salur</span></td>
                    <td></td>
                </tr>
                @endforeach
                <tr class="total-row">
