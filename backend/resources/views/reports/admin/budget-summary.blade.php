...
<body>
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="filter-info">Periode: {{ $filters['tgl_mulai'] ?? '...' }} s/d {{ $filters['tgl_akhir'] ?? '...' }}</div>
            @endif
        </div>
        ...
        <table>
            <thead>
                <tr>
                    <th>No</th>
                    <th>Nama Program</th>
                    <th class="text-right">Benefit (Rp)</th>
                    <th class="text-right">Kuota</th>
                    <th class="text-right">Total Anggaran (Rp)</th>
                    <th class="text-right">Tersalurkan (Rp)</th>
                    <th class="text-right">Sisa (Rp)</th>
                </tr>
            </thead>
            <tbody>
                @php $totalAnggaran = 0; $totalTersalurkan = 0; $totalSisa = 0; @endphp
                @foreach($data as $i => $row)
                @php $totalAnggaran += $row['total_anggaran']; $totalTersalurkan += $row['tersalurkan']; $totalSisa += $row['sisa']; @endphp
                <tr>
                    <td class="text-center">{{ $i + 1 }}</td>
                    <td>{{ $row['program'] }}</td>
                    <td class="text-right">{{ number_format($row['benefit_amount'], 0, ',', '.') }}</td>
                    <td class="text-right">{{ number_format($row['quota_total'], 0, ',', '.') }}</td>
                    <td class="text-right">{{ number_format($row['total_anggaran'], 0, ',', '.') }}</td>
                    <td class="text-right">{{ number_format($row['tersalurkan'], 0, ',', '.') }}</td>
                    <td class="text-right">{{ number_format($row['sisa'], 0, ',', '.') }}</td>
                </tr>
                @endforeach
                <tr class="total-row">
                    <td colspan="4" class="text-right"><strong>TOTAL</strong></td>
                    <td class="text-right"><strong>{{ number_format($totalAnggaran, 0, ',', '.') }}</strong></td>
                    <td class="text-right"><strong>{{ number_format($totalTersalurkan, 0, ',', '.') }}</strong></td>
                    <td class="text-right"><strong>{{ number_format($totalSisa, 0, ',', '.') }}</strong></td>
                </tr>
            </tbody>
        </table>
