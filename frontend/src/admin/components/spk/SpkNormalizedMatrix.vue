<!-- src/admin/components/spk/SpkNormalizedMatrix.vue -->
<template>
  <div>
      ...
      <table class="w-full text-left border-collapse">
        <thead>
          <tr class="bg-[#FAF6F0]/50 text-[#8B7355] text-xs uppercase tracking-wider font-black border-b border-[#E8D5C4]">
            ...
            <th v-for="c in criteria" :key="c.key" class="px-4 py-3 text-center">
              {{ c.label }}<br>
              <span class="font-normal text-[10px] text-[#6B705C]">({{ c.sifat === 'benefit' ? 'Benefit' : 'Cost' }})</span>
            </th>
        ...
        <tbody class="divide-y divide-[#E8D5C4]/30">
          <tr v-for="(row, index) in data" :key="row.id" class="hover:bg-[#FAF6F0]/30">
            ...
            <td v-for="c in criteria" :key="c.key" class="px-4 py-3 text-center">
              <span class="text-[10px] text-[#6B705C] leading-relaxed">{{ getFormulaText(row, c) }}</span>
              <br>
              <span class="text-sm font-black text-[#1B4332]">{{ formatValue(row[c.key]) }}</span>
            </td>
...
import type { SpkCriteria, SpkNormalizedRow } from '../../types/spk'

defineProps<{ data: SpkNormalizedRow[]; criteria: SpkCriteria[] }>()

const formatValue = (val: number | string | undefined): string => typeof val === 'number' ? val.toFixed(4) : String(val ?? '-')
const formatNumber = (val: number): string => new Intl.NumberFormat('id-ID').format(val)

const getFormulaText = (row: SpkNormalizedRow, c: SpkCriteria): string => {
  const raw = row[c.key + '_raw']
  if (typeof raw !== 'number') return '-'
  const ideal = c.ideal_value
  const rawDisplay = formatNumber(raw)
  const idealDisplay = formatNumber(ideal)
  return c.sifat === 'cost' ? `r = (${idealDisplay} - ${rawDisplay}) / ${idealDisplay}` : `r = ${rawDisplay} / ${idealDisplay}`
}
